...
LINKAGE void VUserMain1(void){

    auto proc = new VProc(1); //Wishbone.
    unsigned address;
    uint32_t buffer[16];
    ...
    /* Appears the Vunit Wishbone Slave doesn't handle write byte enables */
    ...
    /* Wishbone doesn't support Burst reads/writes. */
